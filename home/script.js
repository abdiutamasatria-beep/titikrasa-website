...
function renderMenuItems() {
    ...
    menuGrid.innerHTML = MENU_ITEMS.map(([name, category, price, image, description, badge, variants, dietary]) => {
        ...
        return `
        <div class="menu-card" data-name="${name}" data-category="${category}" data-dietary="${dietary}" data-price="${price}" data-desc="${description}" data-badge="${badge || ''}">
...
function setQuickSearchTag(tag, element) {
    activeQuickSearchTag = tag;
    
    // Update active UI state on quick search tag buttons
    ...
    if (tag === 'all') {
        ...
        document.querySelectorAll('.price-filter-btn').forEach(btn => {
            if (btn.getAttribute('data-price-range') === 'all') btn.classList.add('active');
            else btn.classList.remove('active');
        });
    } else if (tag === 'bestseller') {
        if (searchInput) searchInput.value = 'Best Seller';
        currentSearchQuery = 'bestseller';
        if (clearBtn) clearBtn.style.display = 'block';
    } else if (tag === 'under25') {
        if (searchInput) searchInput.value = '< 25rb';
        currentSearchQuery = '';
        filterPriceRange('under25');
...
function handleLiveSearch(event) {
    ...
    // Un-highlight quick tags if user types a custom query
    const tags = document.querySelectorAll('.quick-search-tag');
    let matchedTag = false;
    tags.forEach(t => {
        if (currentSearchQuery === '' && t.innerText.includes('Semua')) {
...
function clearSearchInput() {
    ...
    // Reset quick tag to 'all'
    const tags = document.querySelectorAll('.quick-search-tag');
    tags.forEach(t => {
        if (t.innerText.includes('Semua')) t.classList.add('active');
...
function filterMenuCategory(category) {
    ...
    // Update active state on kitchen pill buttons if applicable
    const halalPill = document.querySelector('.dk-pill.halal');
    const nonHalalPill = document.querySelector('.dk-pill.nonhalal');
    if (halalPill) halalPill.classList.toggle('active', category === 'halal');
    if (nonHalalPill) nonHalalPill.classList.toggle('active', category === 'nonhalal');
...
function applyAllMenuFilters() {
    ...
    cards.forEach(card => {
        ...
        const badge = card.getAttribute('data-badge')?.toLowerCase() || '';

        // 1. Match Query (handles keywords & smart tags)
        let matchQuery = !query;
        if (query) {
            if (query === 'bestseller') {
                matchQuery = (badge === 'bestseller' || badge === 'recommended');
            } else if (query === 'kuah') {
                matchQuery = name.includes('rawon') || name.includes('gulai') || name.includes('kari') || name.includes('papeda') || name.includes('mie aceh') || desc.includes('kuah') || desc.includes('sup');
            } else if (query === 'pedas') {
                matchQuery = desc.includes('pedas') || desc.includes('sambal') || name.includes('betutu') || name.includes('mie aceh') || name.includes('pempek') || cat === 'utama';
            } else {
                matchQuery = name.includes(query) || desc.includes(query) || cat.includes(query);
            }
        }
    ...
    container.innerHTML = reviews.map(rev => {
        ...
        <div class="review-card-modern">
            <div class="review-card-top">
                <div class="reviewer-avatar-group">
                ...
                ${rev.verified ? `<span class="review-verified-badge" title="Tamu Terverifikasi"><i data-lucide="check-circle-2"></i> Terverifikasi</span>` : ''}
            </div>
            <div class="review-dish-tag">
                <i data-lucide="utensils"></i>
                <span>Menu Favorit: <strong>${rev.dish}</strong></span>
            </div>
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 3.1 Initialize Search Dropdown Popover
    initSearchPopover();
